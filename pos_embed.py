...
def get_iso_region_sincos_pos_embed_from_grid(embed_dim, grid, region_ids):
    assert embed_dim % 2 == 0

    # emb_r = get_1d_sincos_pos_embed_from_grid(embed_dim // 2, grid[0])  # (N_r, D/2)
    emb_i = get_1d_sincos_pos_embed_from_grid(embed_dim, grid[1])  # (N_i, D/2)

    # emb_i = emb_i[region_ids]
    # emb = np.concatenate([emb_r, emb_i], axis=1)  # (H*W, D)
    emb = emb_i[region_ids]

    return emb

def get_lat_lon_sincos_pos_embed_from_grid(embed_dim, grid):
    assert embed_dim % 2 == 0

    emb_la = get_1d_sincos_pos_embed_from_grid(embed_dim//2, grid[0])  # (N_i, D/2)
    emb_lo = get_1d_sincos_pos_embed_from_grid(embed_dim//2, grid[1])  # (N_i, D/2)

    # emb_la = emb_la[region_ids]
    # emb_lo = emb_lo[region_ids]
    emb = np.concatenate([emb_la, emb_lo], axis=1)  # (H*W, D)

    return emb

# def get_day_pos_embed_from_grid(pos):
#     assert embed_dim % 2 == 0
